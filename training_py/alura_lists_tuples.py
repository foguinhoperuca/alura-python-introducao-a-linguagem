...
def exerc_10() -> None:
    ...
    Exemplo de Entrada: Digite os dados do aluno no formato Nome, Idade, Nota separados por vírgula: João, 16, 8.5, Maria, 17, 9.2, Pedro, 15, 7.8
    Saída esperada:
        Aluno: João
        Idade: 16
        Nota: 8.5
    ...
    report: List[Tuple[str, int, float]] = [('Jeff', 39, 10.0,), ('Jenifer', 29, 8.5), ('Mike', 69, 7.0)]
    while True:
        name: str = input("Inform student's name: ")
        if name in ('exit', 'e', 'end', '', 'sair', 'salir'):
            break

        age: int = int(input("Inform student's age: "))
        grade: float = float(input("Inform student's grade: "))
        report.append((name, age, grade))
        print(f'{colored("[LISTS_TUPLES][10]", "white", attrs=CGATTRS)} -------------------------')

    for register in report:
        print(f'Student..: {register[0]}')
        print(f'Age......: {register[1]}')
        print(f'Grade....: {register[2]}')
        print(f'{colored("[LISTS_TUPLES][10]", "white", attrs=CGATTRS)} -------------------------')
